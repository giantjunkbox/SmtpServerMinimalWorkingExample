...
namespace SmtpServer.IO
{
    public sealed class NetworkClient : INetworkClient
    {
        ...
        public IReadOnlyList<ArraySegment<byte>> ReadAsync(Func<byte, bool> @continue, long count)
        {
            if (ReadBufferAsync() == false)
            {
        ...
        public void WriteAsync(IReadOnlyList<ArraySegment<byte>> buffers)
        {
        ...
        /// <param name="cancellationToken">The cancellation token.</param>
        /// <returns>A task that asynchronously performs the operation.</returns>
        public void UpgradeAsync(X509Certificate certificate, SslProtocols protocols)
        {
            var stream = new SslStream(_stream, true);

            stream.AuthenticateAsServerAsync(certificate, false, protocols, true);
